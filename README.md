...
```bash
curl -fsSL -o /usr/local/bin/kick-live https://raw.githubusercontent.com/Ka-Ga-R1/kick-live/main/kick-live-manager/kick-live.sh && chmod +x /usr/local/bin/kick-live && kick-live
```
...
```bash
curl -fsSL -o /usr/local/bin/kick-live https://raw.githubusercontent.com/Ka-Ga-R1/kick-live/main/kick-live-manager/kick-live.sh && chmod +x /usr/local/bin/kick-live
```
...
选择 `升级脚本`。脚本会更新主程序、worker、systemd 模板，检查已有直播间配置，修复权限并重新生成播放列表。

如果检测到直播间正在运行，会询问是否重启这些直播间以应用新版本。

从 `v1.0.0` 更新时不要直接使用旧版菜单里的 `升级脚本`，旧版会把 `/usr/local/bin/kick-live` 复制到自身并报错。先按上面的命令覆盖到新版，再进入菜单执行升级迁移。

## 安全说明

- 脚本需要 root 运行，因为要安装软件包、写 Nginx 配置和管理 systemd 服务。
- 拉流服务以 `www-data` 用户运行，不使用 root 跑 `ffmpeg`。
- 配置文件使用普通 key/value 读取，不执行 `.env` 里的 shell 代码。
- 默认域名是示例域名，实际域名只保存在本机 `/etc/kick-live/config.env`。
- 播放列表是公开 URL，建议使用不容易猜到的本地名称，或者自行加访问控制。
